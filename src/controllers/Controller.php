...
abstract class Controller extends PDOConnection
{
    /**
     * DB error logger
     *
    ...
    public function create(array $argument)
    {
        try {
            ...
            $bindparam = $argument["bindparams"] ?? null;

            $result = $this->conn->prepare("INSERT INTO $table ($fields) VALUES ($values)");

            if ($bindparam != null) {
                foreach ($bindparam as $key => $value) {
                    $result->bindParam($key, $value, $this->bind($value));
                }
            }
            return ["status" => $result->execute(), "message" => "Record added successfully"];
    ...
    public function findOne(array $argument)
    {
        try {
            ...
            $result->execute();
            return ["status" => true, "message" => "Record found", "data" => $result->fetch()];
        } catch (Exception $error) {
            $this->errorhandler($error->getMessage());
            return ["status" => false, "message" => $error->getMessage()];
        }
    ...
    public function findAll(array $argument)
    {
        try {
            ...
            $result->execute();
            return ["status" => true, "message" => "Records found", "data" => $result->fetchAll()];
        } catch (Exception $error) {
            $this->errorhandler($error->getMessage());
            return ["status" => false, "message" => $error->getMessage()];
        }
    ...
    public function update(array $argument)
    {
        try {
            ...
            $stmt = $this->conn->exec($result);

            return ["status" => $stmt == 0 ? false : true, "message" => "$stmt Record updated successfully"];
        } catch (Exception $error) {
            $this->errorhandler($error->getMessage());
            return ["status" => false, "message" => $error->getMessage()];
        }
    ...
    public function destroy(array $argument)
    {
        try {
            ...
            return ["status" => $stmt == 0 ? false : true, "message" => "$stmt Record deleted successfully"];
        } catch (Exception $error) {
            $this->errorhandler($error->getMessage());
            return ["status" => false, "message" => $error->getMessage()];
        }
    ...
    public function getCount(array $argument)
    {
        try {
            ...
            $result->execute();
            return ["status" => true, "message" => "success", "data" => $result->rowCount()];
        } catch (Exception $error) {
            $this->errorhandler($error->getMessage());
            return ["status" => false, "message" => $error->getMessage()];
        }
